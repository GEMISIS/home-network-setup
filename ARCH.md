...
| Camera segment | PoE switch (optional) | Uplink un/tagged VLAN 60; isolates cameras. |
| Home‑office link | Direct 10 G DAC / RJ‑45 | VLAN 70; management PC or small switch. |
| Endpoint pools | • Media (VLAN 50)  <br>• Home Assistant (VLAN 51) | Access ports on core switch. |
...
|---|---------|-----------------------|
| **1** | **Default-deny firewall** | *NixOS* sets `networking.firewall.enable = true` by default (nftables backend). Add explicit rules for:<br>• Inter-VLAN flows (HA ↔ Automation, HA ↔ Cameras)<br>• NAT-to-WAN for VLAN 10 / 30 / 40 / 50 / 51 / 70<br>Everything else → `drop`. |
| **2** | **Camera VLAN isolation** | Omit any rule that allows `192.168.60.0/24 -> enp1s0`.<br>Add explicit `reject with icmpx` to document intent. |
| **3** | **Admin-plane isolation** | • Keep SSH/HTTPS services bound only to `192.168.70.1` and WireGuard interface `wg0`.<br>• Switch ports for management PCs = **untagged VLAN 70** only.<br>• WireGuard peers: `AllowedIPs = 192.168.70.0/24`. |
| **4** | **Timely patching** | Enable automated upgrades: <br>`services.nixos-upgrade = { enable = true; timerConfig.OnCalendar = "daily"; };`<br>– or –<br>run a nightly `systemd` timer that executes `nixos-rebuild switch --upgrade` against your flake. |
| **5** | **Central logging** | • Enable Grafana/Loki or Elastic in VLAN 70.<br>• On router: `services.promtail.enable = true; services.promtail.lokiAddresses = ["http://logbox:3100"];`<br>– or –<br>`services.journalbeat.enable = true;` and ship to Elastic. |
| **6** | **Intrusion prevention** | Deploy CrowdSec with nftables bouncer:<br>`services.crowdsec = { enable = true; postOverflows = [ "nftables" ]; };` |
| **7** | **802.1X port authentication (optional)** | If switch supports it, run RADIUS:<br>`services.freeradius.enable = true;`<br>Set required ports (`office`, `WAP-uplink`) to 802.1X + MAC-auth bypass as needed. |
| **10** | **VLAN-hopping prevention** | • On every trunk: set **native VLAN = none/4095**.<br>• Ensure WAP SSID-to-VLAN mapping is explicit, with trunk native disabled.<br>• Audit switch config for any stray `pvid` on trunk ports. |
